...
# (8081), not the docker-compose bunker-api container (8080) — that's what
...
export DATABASE_URL="postgres://bunker:${POSTGRES_PASSWORD:-bunker}@localhost:5432/bunker"
export JWT_SECRET="local-dev-secret-not-for-production"
export BUNKER_BASE_DOMAIN="${BUNKER_BASE_DOMAIN:-bunker.localhost}"
...
export BUNKER_BOOTSTRAP_ADMIN_USERNAME="admin"
export BUNKER_BOOTSTRAP_ADMIN_PASSWORD="admin123"
...
sqlx migrate run --source crates/bunker-infrastructure/migrations

echo "==> Starting backend (cargo run -p bunker-api, port 8081)"
