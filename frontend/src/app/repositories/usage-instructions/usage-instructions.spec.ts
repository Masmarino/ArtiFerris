...
const FAKE_LOCATION = { origin: 'http://artiferris.test:8080', host: 'artiferris.test:8080' }
const FAKE_DOCUMENT = {
...
describe('UsageInstructions', () => {
  it('shows docker login, tag, push and pull for a hosted docker repository', () => {
    ...
    expect(text).toContain('docker login artiferris.test:8080')
    expect(text).toContain('--password-stdin')
    expect(text).toContain('docker tag')
    expect(text).toContain('docker push artiferris.test:8080/my-images/')
    expect(text).toContain('docker pull artiferris.test:8080/my-images/')
  })
  ...
  it('shows docker login and pull only for a proxy docker repository, never push', () => {
    ...
    expect(text).toContain('docker login artiferris.test:8080')
    expect(text).toContain('docker pull artiferris.test:8080/docker-hub-proxy/')
  ...
  it('shows the .npmrc block and publish/install for a hosted npm repository', () => {
    ...
    expect(text).toContain('registry=http://artiferris.test:8080/npm/my-packages/')
    expect(text).toContain('//artiferris.test:8080/npm/my-packages/:_authToken=<votre-token>')
  ...
  it('shows the .npmrc block and install only for a group npm repository, never publish', () => {
    ...
    expect(text).toContain('registry=http://artiferris.test:8080/npm/aggregated/')
    expect(text).toContain('npm install')
