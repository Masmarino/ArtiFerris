...
fn merge_scopes(raw_scopes: &[String]) -> Option<String> {
    let mut merged: Option<bunker_domain::docker_registry::DockerScopeRequest> = None;
    for raw in raw_scopes {
        let Some(parsed) = bunker_domain::docker_registry::DockerScopeRequest::parse(raw) else { continue };
        match &mut merged {
...
mod tests {
    ...
    use bunker_domain::organization::PUBLIC_ORGANIZATION_ID;
