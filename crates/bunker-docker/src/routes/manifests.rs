...
use bunker_domain::docker_registry::{Digest, DockerImageName, DockerMediaType};
use uuid::Uuid;
...
pub async fn get_manifest(
    ...
    match state.get_manifest.execute(repo.id, &image_name, &reference).await {
        Ok(Some(manifest)) => {
            if repo.repo_type == bunker_domain::package_repository::RepositoryType::Proxy {
...
mod tests {
    ...
    async fn a_manifest_in_one_organizations_repository_is_not_reachable_from_another_organization(pool: sqlx::PgPool) {
        ...
        state
            ...
            .create(&bunker_domain::organization::Organization {
                id: other_id,
                slug: bunker_domain::organization::OrganizationSlug::parse("other").unwrap(),
