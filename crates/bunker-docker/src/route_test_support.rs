...
use bunker_application::use_cases::api_token::hash_api_token;
...
use bunker_infrastructure::filesystem_docker_blob_store::FilesystemDockerBlobStore;
use bunker_infrastructure::http_remote_docker_registry::HttpRemoteDockerRegistry;
use bunker_infrastructure::jwt_docker_token_issuer::JwtDockerTokenIssuer;
...
use bunker_infrastructure::postgres::docker_manifest_repository::PostgresDockerManifestRepository;
use bunker_infrastructure::postgres::docker_upload_session_repository::PostgresDockerUploadSessionRepository;
use bunker_infrastructure::trivy_docker_image_scanner::TrivyDockerImageScanner;
...
use bunker_domain::docker_registry::DockerGrantedScope;
use sqlx::PgPool;
use uuid::Uuid;
...
pub async fn test_state(pool: PgPool, root: &Path) -> DockerState {
    ...
    let remote: Arc<dyn bunker_domain::docker_remote::RemoteDockerRegistryPort> = Arc::new(HttpRemoteDockerRegistry::new());
    let token_issuer: Arc<dyn bunker_domain::docker_registry::DockerTokenIssuerPort> = Arc::new(JwtDockerTokenIssuer::new("test-secret".to_string()));
    let events: Arc<dyn bunker_domain::audit::EventPublisherPort> = Arc::new(PostgresEventPublisher::new(pool.clone()));
    let docker_image_scans: Arc<dyn bunker_domain::docker_scan::DockerImageScanRepositoryPort> = Arc::new(PostgresDockerImageScanRepository::new(pool.clone()));
    let docker_scanner: Arc<dyn bunker_domain::docker_scan::DockerImageScannerPort> = Arc::new(TrivyDockerImageScanner::new("127.0.0.1:0".to_string()));
...
pub fn issue_test_token(state: &DockerState, user_id: Uuid, repository_id: Uuid, repository_name: &str, image_name: &str, actions: &[&str]) -> String {
    issue_test_token_for_org(state, user_id, bunker_domain::organization::PUBLIC_ORGANIZATION_ID, false, repository_id, repository_name, image_name, actions)
}
