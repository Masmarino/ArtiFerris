...
fn rp_origin_url(artiferris_base_domain: &str, public_url: &str) -> Result<Url, String> {
    let scheme = if artiferris_base_domain.starts_with("localhost") { "http" } else { "https" };
    let port_suffix = Url::parse(public_url).ok().and_then(|u| u.port()).map(|p| format!(":{p}")).unwrap_or_default();
    Url::parse(&format!("{scheme}://{artiferris_base_domain}{port_suffix}")).map_err(|e| format!("BUNKER_BASE_DOMAIN ({artiferris_base_domain}) is not usable as a URL: {e}"))
}
...
pub fn build_webauthn_client(artiferris_base_domain: &str, rp_name: &str, public_url: &str) -> Result<Webauthn, String> {
    ...
    WebauthnBuilder::new(artiferris_base_domain, &rp_origin)
        .map_err(|e| format!("BUNKER_BASE_DOMAIN ({artiferris_base_domain}) is not usable as a WebAuthn relying party: {e}"))?
        .rp_name(rp_name)
