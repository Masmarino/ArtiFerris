...
use artiferris_domain::npm_package::{NpmDistTag, NpmPackageName, NpmPackageRepositoryPort, NpmPackageVersion};
            ...
            match self.remote.fetch_metadata(remote_url, name, repo.remote_username.as_deref(), repo.remote_password.as_deref()).await {
                Ok(document) => {
                    let package_id = match &existing {
                        Some(p) => p.id,
                        None => {
                            let created = artiferris_domain::npm_package::NpmPackage {
                                id: Uuid::new_v4(),
