...
use artiferris_domain::audit::{AuditEntry, AuditQueryFilter, EventPublisherPort, SecurityEvent};
...
impl AdminListApiTokensUseCase {
    pub fn new(tokens: Arc<dyn artiferris_domain::api_token::ApiTokenRepositoryPort>, users: Arc<dyn UserRepositoryPort>) -> Self {
        Self { tokens, users }
    }
    ...
    pub async fn execute(&self) -> Result<Vec<AdminApiTokenEntry>, ApplicationError> {
        ...
        let user_by_id: std::collections::HashMap<Uuid, &artiferris_domain::user::User> = users.iter().map(|u| (u.id, u)).collect();
        Ok(tokens
            .into_iter()
            .map(|t| {
...
pub struct ImportConfigurationUseCase {
    ...
    repositories: Arc<dyn PackageRepositoryQueryPort>,
    invitations: Arc<dyn artiferris_domain::invitation::UserInvitationPort>,
...
impl ImportConfigurationUseCase {
    ...
    pub fn new(
        ...
        Self { users, hasher, create_repository, set_quota, set_retention, add_group_member, grant_permission, update_settings, repositories, invitations, email, organizations, artiferris_base_domain }
    }
    ...
    pub async fn execute(&self, import: ConfigurationImport, actor_id: Uuid) -> Result<ImportReport, ApplicationError> {
        ...
        let activation_origin = crate::use_cases::invitation::organization_origin(&self.artiferris_base_domain, &restore_organization);
        ...
        for exported in &import.users {
            let username = match artiferris_domain::user::Username::parse(&exported.username) {
                Ok(u) => u,
            ...
            let user = artiferris_domain::user::User {
                id: exported.id,
                username,
                password_hash,
        ...
        for exported in &import.users {
            ...
            let invitation = artiferris_domain::invitation::UserInvitation {
                user_id: exported.id,
...
mod tests {
    ...
    impl artiferris_domain::invitation::UserInvitationPort for FakeInvitations {
        async fn upsert(&self, invitation: &artiferris_domain::invitation::UserInvitation) -> Result<(), DomainError> {
    ...
    impl DockerBlobStorePort for FakeDockerBlobs {
        ...
        async fn decrement_ref_and_delete_if_zero(&self, _digest: &artiferris_domain::docker_registry::Digest) -> Result<bool, DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
    ...
    struct FakeApiTokens {
        tokens: Mutex<Vec<artiferris_domain::api_token::ApiToken>>,
    }

    #[async_trait]
    impl artiferris_domain::api_token::ApiTokenRepositoryPort for FakeApiTokens {
        async fn insert(&self, token: &artiferris_domain::api_token::ApiToken) -> Result<(), DomainError> {
            self.tokens.lock().unwrap().push(token.clone());
            Ok(())
        }
        async fn list_for_user(&self, user_id: Uuid) -> Result<Vec<artiferris_domain::api_token::ApiToken>, DomainError> {
            Ok(self.tokens.lock().unwrap().iter().filter(|t| t.user_id == user_id).cloned().collect())
        }
        async fn list_all(&self) -> Result<Vec<artiferris_domain::api_token::ApiToken>, DomainError> {
            Ok(self.tokens.lock().unwrap().clone())
        }
        async fn find_by_hash(&self, token_hash: &str) -> Result<Option<artiferris_domain::api_token::ApiToken>, DomainError> {
    ...
    fn sample_token(user_id: Uuid, label: &str) -> artiferris_domain::api_token::ApiToken {
        artiferris_domain::api_token::ApiToken {
            id: Uuid::new_v4(),
            user_id,
