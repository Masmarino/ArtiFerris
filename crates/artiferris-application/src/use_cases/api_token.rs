...
mod tests {
    ...
    impl artiferris_domain::api_token::ApiTokenRepositoryPort for FakeTokens {
        ...
        async fn list_all(&self) -> Result<Vec<artiferris_domain::api_token::ApiToken>, artiferris_domain::error::DomainError> {
        ...
        async fn revoke_any(&self, id: Uuid) -> Result<(), artiferris_domain::error::DomainError> {
            if let Some(t) = self.tokens.lock().unwrap().get_mut(&id) {
                t.revoked_at = Some(chrono::Utc::now());
            }
            Ok(())
