...
use bunker_domain::email::{SmtpSecurity, SmtpSettings, SmtpSettingsPort};
...
mod tests {
    ...
    async fn get_returns_none_when_never_configured(pool: sqlx::PgPool) {
        ...
        assert_eq!(repo.get(bunker_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap(), None);
    }

    #[sqlx::test(migrations = "../bunker-infrastructure/migrations")]
    async fn update_then_get_round_trips_including_the_password(pool: sqlx::PgPool) {
        let repo = PostgresSmtpSettingsRepository::new(pool, "jwt-secret".to_string());
        repo.update(bunker_domain::organization::PUBLIC_ORGANIZATION_ID, &sample()).await.unwrap();

        assert_eq!(repo.get(bunker_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap(), Some(sample()));
    }
    ...
    async fn a_second_update_overwrites_the_first_rather_than_inserting_a_row(pool: sqlx::PgPool) {
        ...
        assert_eq!(count, 1);
        assert_eq!(repo.get(bunker_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap().unwrap().host, "smtp2.example.com");
    }
    ...
    async fn the_password_is_never_stored_in_plaintext(pool: sqlx::PgPool) {
        ...
        let row: (Vec<u8>,) = sqlx::query_as("SELECT encrypted_password FROM smtp_settings WHERE organization_id = $1").bind(bunker_domain::organization::PUBLIC_ORGANIZATION_ID).fetch_one(&repo.pool).await.unwrap();
    ...
    async fn settings_for_one_organization_do_not_affect_another(pool: sqlx::PgPool) {
        ...
        assert_eq!(repo.get(bunker_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap(), None, "the public organization must still be unconfigured");
