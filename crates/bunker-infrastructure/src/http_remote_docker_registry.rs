...
/// Matches `bunker-docker`'s own manifest-push cap (`dispatch::MANIFEST_BODY_LIMIT_BYTES`).
const MAX_MANIFEST_RESPONSE_BYTES: usize = 10 * 1024 * 1024;
/// Matches `bunker-docker`'s own blob-upload cap (`BLOB_BODY_LIMIT_BYTES`).
...
mod tests {
    ...
    async fn fetches_a_real_manifest_from_docker_hub_via_the_bearer_challenge_flow() {
        ...
        let image_name = bunker_domain::docker_registry::DockerImageName::parse("library/alpine").unwrap();
