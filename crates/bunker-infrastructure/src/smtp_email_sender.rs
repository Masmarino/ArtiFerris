...
use bunker_application::email_templates::LOGO_CID;
use bunker_domain::branding::BrandingPort;
use bunker_domain::email::{EmailPort, SmtpSecurity, SmtpSettingsPort};
...
mod tests {
    ...
    use bunker_domain::branding::{BrandingAsset, BrandingSettings};

    use super::*;

    struct FakeSmtpSettings {
        settings: Mutex<std::collections::HashMap<Uuid, bunker_domain::email::SmtpSettings>>,
    }

    #[async_trait]
    impl SmtpSettingsPort for FakeSmtpSettings {
        async fn get(&self, organization_id: Uuid) -> Result<Option<bunker_domain::email::SmtpSettings>, DomainError> {
            Ok(self.settings.lock().unwrap().get(&organization_id).cloned())
        }
        async fn update(&self, organization_id: Uuid, settings: &bunker_domain::email::SmtpSettings) -> Result<(), DomainError> {
    ...
    fn sample_smtp_settings() -> bunker_domain::email::SmtpSettings {
        bunker_domain::email::SmtpSettings {
    ...
    async fn a_host_pointing_at_a_private_address_is_rejected_before_connecting() {
        ...
        let mut settings_by_org = std::collections::HashMap::new();
        settings_by_org.insert(org_id, bunker_domain::email::SmtpSettings { host: "127.0.0.1".to_string(), ..sample_smtp_settings() });
        let settings = FakeSmtpSettings { settings: Mutex::new(settings_by_org) };
    ...
    async fn a_host_pointing_at_the_cloud_metadata_endpoint_is_rejected() {
        let org_id = Uuid::new_v4();
        let mut settings_by_org = std::collections::HashMap::new();
        settings_by_org.insert(org_id, bunker_domain::email::SmtpSettings { host: "169.254.169.254".to_string(), ..sample_smtp_settings() });
        let settings = FakeSmtpSettings { settings: Mutex::new(settings_by_org) };
