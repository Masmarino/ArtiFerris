...
use bunker_domain::sso::ExternalIdentity;
use bunker_domain::user::{PasswordHasherPort, TokenIssuerPort, User, UserRepositoryPort, Username};
use uuid::Uuid;
...
pub struct ProvisionSsoUserUseCase {
    ...
    system_settings: Arc<dyn bunker_domain::system_settings::SystemSettingsPort>,
}
...
mod tests {
    ...
    impl TokenIssuerPort for FakeTokenIssuer {
        ...
        fn verify(&self, token: &str) -> Result<bunker_domain::user::VerifiedToken, DomainError> {
            ...
            Ok(bunker_domain::user::VerifiedToken { user_id, issued_at: chrono::Utc::now() })
        }
    }
    ...
    #[async_trait]
    impl bunker_domain::system_settings::SystemSettingsPort for FakeSystemSettings {
        async fn get(&self, _organization_id: Uuid) -> Result<bunker_domain::system_settings::SystemSettings, DomainError> {
            Ok(bunker_domain::system_settings::SystemSettings::defaults())
        }
        async fn update(&self, _organization_id: Uuid, _settings: &bunker_domain::system_settings::SystemSettings) -> Result<(), DomainError> {
            Ok(())
        }
    }
    ...
    async fn reusing_a_super_admin_account_from_a_different_organization_is_rejected_against_a_real_database(pool: sqlx::PgPool) {
        let organizations = Arc::new(bunker_infrastructure::postgres::organization_repository::PostgresOrganizationRepository::new(pool.clone()));
        let organization_a = Uuid::new_v4();
        let organization_b = Uuid::new_v4();
        organizations
        ...
        organizations
            .create(&bunker_domain::organization::Organization {
                id: organization_b,
                slug: bunker_domain::organization::OrganizationSlug::parse("globex").unwrap(),
        ...
        let users = Arc::new(bunker_infrastructure::postgres::user_repository::PostgresUserRepository::new(pool.clone()));
        users
            .insert(&User {
                id: Uuid::new_v4(),
