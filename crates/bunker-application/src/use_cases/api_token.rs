...
use base64::Engine;
use chrono::Utc;
use bunker_domain::api_token::{ApiToken, ApiTokenRepositoryPort};
use rand::Rng;
...
mod tests {
    ...
    struct FakeTokens {
        tokens: Mutex<HashMap<Uuid, bunker_domain::api_token::ApiToken>>,
    }
    impl FakeTokens {
        fn new() -> Self { Self { tokens: Mutex::new(HashMap::new()) } }
    }
    #[async_trait::async_trait]
    impl bunker_domain::api_token::ApiTokenRepositoryPort for FakeTokens {
        async fn insert(&self, token: &bunker_domain::api_token::ApiToken) -> Result<(), bunker_domain::error::DomainError> {
            self.tokens.lock().unwrap().insert(token.id, token.clone());
            Ok(())
        }
        async fn list_for_user(&self, user_id: Uuid) -> Result<Vec<bunker_domain::api_token::ApiToken>, bunker_domain::error::DomainError> {
            Ok(self.tokens.lock().unwrap().values().filter(|t| t.user_id == user_id).cloned().collect())
        }
        async fn list_all(&self) -> Result<Vec<bunker_domain::api_token::ApiToken>, bunker_domain::error::DomainError> {
            Ok(self.tokens.lock().unwrap().values().cloned().collect())
        }
        async fn find_by_hash(&self, token_hash: &str) -> Result<Option<bunker_domain::api_token::ApiToken>, bunker_domain::error::DomainError> {
            Ok(self.tokens.lock().unwrap().values().find(|t| t.token_hash == token_hash).cloned())
        }
        async fn touch_last_used_at(&self, id: Uuid, used_at: chrono::DateTime<chrono::Utc>) -> Result<(), bunker_domain::error::DomainError> {
            if let Some(t) = self.tokens.lock().unwrap().get_mut(&id) { t.last_used_at = Some(used_at); }
            Ok(())
        }
        async fn revoke(&self, id: Uuid, user_id: Uuid) -> Result<bool, bunker_domain::error::DomainError> {
        ...
        async fn revoke_any(&self, id: Uuid) -> Result<(), bunker_domain::error::DomainError> {
            if let Some(t) = self.tokens.lock().unwrap().get_mut(&id) {
                t.revoked_at = Some(chrono::Utc::now());
            }
            Ok(())
