...
use bunker_domain::user::{PasswordHasherPort, UserRepositoryPort};
use bunker_domain::webauthn::{WebauthnCredential, WebauthnCredentialPort};
use uuid::Uuid;
...
fn deserialize_passkey(credential: &WebauthnCredential) -> Result<Passkey, ApplicationError> {
    serde_json::from_slice(&credential.passkey_data).map_err(|e| ApplicationError::Domain(bunker_domain::error::DomainError::Infrastructure(format!("corrupted stored passkey: {e}"))))
}
...
impl StartPasskeyRegistrationUseCase {
    ...
    pub async fn execute(&self, user_id: Uuid, username: &str) -> Result<(Uuid, CreationChallengeResponse), ApplicationError> {
        ...
        let (ccr, registration) = webauthn
            ...
            .map_err(|e| ApplicationError::Domain(bunker_domain::error::DomainError::Infrastructure(e.to_string())))?;
...
impl StartPasskeyAuthenticationUseCase {
    ...
    pub async fn execute(&self, user_id: Uuid) -> Result<(Uuid, RequestChallengeResponse), ApplicationError> {
        ...
        let (rcr, authentication) =
            webauthn.start_passkey_authentication(&passkeys).map_err(|e| ApplicationError::Domain(bunker_domain::error::DomainError::Infrastructure(e.to_string())))?;
