...
use bunker_domain::docker_registry::{Digest, DockerImageName, DockerManifestRepositoryPort};
use bunker_domain::npm_package::NpmPackageRepositoryPort;
use bunker_domain::package_repository::{PackageRepositoryQueryPort, RepositoryFormat};
use uuid::Uuid;
...
mod tests {
    ...
    use bunker_domain::docker_registry::{Digest, DockerManifest, DockerMediaType};
    use bunker_domain::npm_package::{NpmPackage, NpmPackageName, NpmPackageOrigin, NpmPackageVersion, NpmVersion};
    use bunker_domain::package_repository::{PackageRepositorySummary, RepositoryType};
    ...
    fn use_case_with(
        repositories: Arc<dyn bunker_domain::package_repository::PackageRepositoryQueryPort>,
    ...
    fn docker_manifest(repository_id: Uuid, name: &bunker_domain::docker_registry::DockerImageName, content: &[u8], created_at: chrono::DateTime<Utc>) -> DockerManifest {
        DockerManifest {
            id: Uuid::new_v4(),
            package_repository_id: repository_id,
    ...
    async fn prunes_docker_tags_beyond_the_keep_count_but_never_latest() {
        ...
        let image_name = bunker_domain::docker_registry::DockerImageName::parse("my-app").unwrap();
        let now = Utc::now();
