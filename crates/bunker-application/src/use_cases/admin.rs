...
use bunker_domain::audit::{AuditEntry, AuditQueryFilter, EventPublisherPort, SecurityEvent};
use bunker_domain::docker_registry::DockerBlobStorePort;
use bunker_domain::health::{ComponentHealth, DatabaseHealth, HealthCheckPort};
use bunker_domain::metrics_snapshot::{MetricsSnapshot, MetricsSnapshotRepositoryPort};
use bunker_domain::organization::OrganizationRepositoryPort;
use bunker_domain::package_repository::{PackageRepositoryQueryPort, RepositoryFormat, RepositoryType};
use bunker_domain::permission::{PermissionQueryPort, Role};
use bunker_domain::storage::StorageBackendPort;
use bunker_domain::system_settings::{SystemSettings, SystemSettingsPort};
...
impl GetHealthStatusUseCase {
    ...
    pub async fn execute(&self) -> HealthStatus {
        ...
        let space = self.storage.volume_space().await.unwrap_or(bunker_domain::storage::VolumeSpace { total_bytes: 0, free_bytes: 0 });
...
pub struct AdminListApiTokensUseCase {
    tokens: Arc<dyn bunker_domain::api_token::ApiTokenRepositoryPort>,
    users: Arc<dyn UserRepositoryPort>,
}

impl AdminListApiTokensUseCase {
    pub fn new(tokens: Arc<dyn bunker_domain::api_token::ApiTokenRepositoryPort>, users: Arc<dyn UserRepositoryPort>) -> Self {
        Self { tokens, users }
    }
    ...
    pub async fn execute(&self) -> Result<Vec<AdminApiTokenEntry>, ApplicationError> {
        ...
        let user_by_id: std::collections::HashMap<Uuid, &bunker_domain::user::User> = users.iter().map(|u| (u.id, u)).collect();
        Ok(tokens
            .into_iter()
            .map(|t| {
...
impl AdminRevokeApiTokenUseCase {
    pub fn new(tokens: Arc<dyn bunker_domain::api_token::ApiTokenRepositoryPort>) -> Self {
        Self { tokens }
    }
...
impl ExportConfigurationUseCase {
    ...
    pub async fn execute(&self) -> Result<ConfigurationExport, ApplicationError> {
        ...
        let system_settings = self.system_settings.get(bunker_domain::organization::PUBLIC_ORGANIZATION_ID).await?;
...
pub struct ImportConfigurationUseCase {
    users: Arc<dyn UserRepositoryPort>,
    hasher: Arc<dyn bunker_domain::user::PasswordHasherPort>,
...
impl ImportConfigurationUseCase {
    ...
    pub async fn execute(&self, import: ConfigurationImport, actor_id: Uuid) -> Result<ImportReport, ApplicationError> {
        ...
        for exported in &import.users {
            let username = match bunker_domain::user::Username::parse(&exported.username) {
                Ok(u) => u,
            ...
            let user = bunker_domain::user::User {
                id: exported.id,
                username,
                password_hash,
        ...
        for exported in &import.users {
            ...
            let invitation = bunker_domain::invitation::UserInvitation {
                user_id: exported.id,
...
mod tests {
    ...
    use bunker_domain::error::{DomainError, EventStoreError};
    use bunker_domain::organization::{Organization, OrganizationSlug};
    use bunker_domain::package_repository::{PackageRepositorySummary, RepositoryFormat, RepositoryType};
    use bunker_domain::permission::Role;
    use bunker_domain::storage::StorageError;
    use bunker_domain::user::{User, Username};
    ...
    use bunker_domain::invitation::UserInvitationPort;
    use bunker_domain::permission::PermissionEventStorePort;
    ...
    struct FakePackageRepositoryStore {
        streams: Mutex<HashMap<Uuid, Vec<bunker_domain::package_repository::PackageRepositoryEvent>>>,
    }
    ...
    impl FakePackageRepositoryStore {
        ...
        fn summarize(id: Uuid, events: &[bunker_domain::package_repository::PackageRepositoryEvent]) -> Option<PackageRepositorySummary> {
            let repo = bunker_domain::package_repository::PackageRepository::from_events(events);
            ...
            let organization_id = events
                ...
                .find_map(|event| match event {
                    bunker_domain::package_repository::PackageRepositoryEvent::Created { organization_id, .. } => Some(*organization_id),
                    _ => None,
                })
    ...
    #[async_trait]
    impl bunker_domain::package_repository::PackageRepositoryEventStorePort for FakePackageRepositoryStore {
        async fn load(&self, repository_id: Uuid) -> Result<(u64, Vec<bunker_domain::package_repository::PackageRepositoryEvent>), EventStoreError> {
        ...
        async fn append(
            ...
            events: Vec<bunker_domain::package_repository::PackageRepositoryEvent>,
            _actor_id: Uuid,
    ...
    struct FakePermissionStore {
        streams: Mutex<HashMap<(Uuid, Uuid), Vec<bunker_domain::permission::PermissionEvent>>>,
    }
    ...
    #[async_trait]
    impl bunker_domain::permission::PermissionEventStorePort for FakePermissionStore {
        async fn load(&self, user_id: Uuid, repository_id: Uuid) -> Result<(u64, Vec<bunker_domain::permission::PermissionEvent>), EventStoreError> {
            let streams = self.streams.lock().unwrap();
        ...
        async fn append(
            ...
            repository_id: Uuid,
            expected_version: u64,
            events: Vec<bunker_domain::permission::PermissionEvent>,
            _actor_id: Uuid,
        ) -> Result<(), EventStoreError> {
            let mut streams = self.streams.lock().unwrap();
    ...
    struct FakeInvitations {
        by_user: Mutex<HashMap<Uuid, bunker_domain::invitation::UserInvitation>>,
    }
    ...
    #[async_trait]
    impl bunker_domain::invitation::UserInvitationPort for FakeInvitations {
        async fn upsert(&self, invitation: &bunker_domain::invitation::UserInvitation) -> Result<(), DomainError> {
            self.by_user.lock().unwrap().insert(invitation.user_id, invitation.clone());
            Ok(())
        }
        async fn find_by_token_hash(&self, token_hash: &str) -> Result<Option<bunker_domain::invitation::UserInvitation>, DomainError> {
            Ok(self.by_user.lock().unwrap().values().find(|i| i.token_hash == token_hash).cloned())
        }
        async fn find_by_user_id(&self, user_id: Uuid) -> Result<Option<bunker_domain::invitation::UserInvitation>, DomainError> {
    ...
    #[async_trait]
    impl bunker_domain::user::PasswordHasherPort for FakeHasher {
    ...
    #[async_trait]
    impl bunker_domain::email::EmailPort for FakeEmail {
    ...
    impl EventPublisherPort for FakeEventPublisher {
        ...
        async fn publish_npm_event(
            &self,
            _event: bunker_domain::audit::NpmPackageEvent,
        ...
        async fn publish_docker_event(
            &self,
            _event: bunker_domain::audit::DockerRegistryEvent,
    ...
    impl DockerBlobStorePort for FakeDockerBlobs {
        async fn write(&self, _digest: &bunker_domain::docker_registry::Digest, _bytes: &[u8]) -> Result<(), DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
        async fn adopt_staged_file(&self, _digest: &bunker_domain::docker_registry::Digest, _staging_path: &str, _size_bytes: u64) -> Result<(), DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
        async fn read(&self, _digest: &bunker_domain::docker_registry::Digest) -> Result<Vec<u8>, DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
        async fn read_stream(&self, _digest: &bunker_domain::docker_registry::Digest) -> Result<bunker_domain::docker_registry::ByteStream, DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
        async fn link_to_repository(&self, _repository_id: Uuid, _digest: &bunker_domain::docker_registry::Digest) -> Result<(), DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
        async fn is_uploaded_to_repository(&self, _repository_id: Uuid, _digest: &bunker_domain::docker_registry::Digest) -> Result<bool, DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
        async fn exists(&self, _digest: &bunker_domain::docker_registry::Digest) -> Result<bool, DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
        async fn size_if_exists(&self, _digest: &bunker_domain::docker_registry::Digest) -> Result<Option<u64>, DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
        async fn existing_digests(&self, _digests: &[bunker_domain::docker_registry::Digest]) -> Result<std::collections::HashSet<String>, DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
        async fn sum_sizes(&self, _digests: &[bunker_domain::docker_registry::Digest]) -> Result<u64, DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
        async fn increment_ref(&self, _digest: &bunker_domain::docker_registry::Digest) -> Result<(), DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
        async fn increment_ref_all(&self, _digests: &[bunker_domain::docker_registry::Digest]) -> Result<(), DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
        async fn decrement_ref_and_delete_if_zero(&self, _digest: &bunker_domain::docker_registry::Digest) -> Result<bool, DomainError> {
            unreachable!("not exercised by GetUsageMetricsUseCase's tests")
        }
    ...
    impl StorageBackendPort for FakeStorage {
        ...
        async fn read_stream(&self, _repository_id: Uuid, _path: &str) -> Result<bunker_domain::storage::ByteStream, StorageError> {
        ...
        async fn volume_space(&self) -> Result<bunker_domain::storage::VolumeSpace, StorageError> {
            Ok(bunker_domain::storage::VolumeSpace { total_bytes: 1000, free_bytes: 400 })
        }
    }
    ...
    struct FakeApiTokens {
        tokens: Mutex<Vec<bunker_domain::api_token::ApiToken>>,
    }

    #[async_trait]
    impl bunker_domain::api_token::ApiTokenRepositoryPort for FakeApiTokens {
        async fn insert(&self, token: &bunker_domain::api_token::ApiToken) -> Result<(), DomainError> {
            self.tokens.lock().unwrap().push(token.clone());
            Ok(())
        }
        async fn list_for_user(&self, user_id: Uuid) -> Result<Vec<bunker_domain::api_token::ApiToken>, DomainError> {
            Ok(self.tokens.lock().unwrap().iter().filter(|t| t.user_id == user_id).cloned().collect())
        }
        async fn list_all(&self) -> Result<Vec<bunker_domain::api_token::ApiToken>, DomainError> {
            Ok(self.tokens.lock().unwrap().clone())
        }
        async fn find_by_hash(&self, token_hash: &str) -> Result<Option<bunker_domain::api_token::ApiToken>, DomainError> {
    ...
    fn sample_token(user_id: Uuid, label: &str) -> bunker_domain::api_token::ApiToken {
        bunker_domain::api_token::ApiToken {
            id: Uuid::new_v4(),
            user_id,
