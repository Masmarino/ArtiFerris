...
use bunker_domain::npm_package::{NpmDistTag, NpmPackageName, NpmPackageRepositoryPort, NpmPackageVersion};
            ...
            match self.remote.fetch_metadata(remote_url, name, repo.remote_username.as_deref(), repo.remote_password.as_deref()).await {
                Ok(document) => {
                    let package_id = match &existing {
                        Some(p) => p.id,
                        None => {
                            let created = bunker_domain::npm_package::NpmPackage {
                                id: Uuid::new_v4(),
...
mod tests {
    ...
    use chrono::Utc;
    use bunker_domain::npm_package::NpmPackageName;
    ...
    async fn a_hosted_repository_serves_its_own_published_versions() {
        ...
        let package = bunker_domain::npm_package::NpmPackage {
            id: Uuid::new_v4(),
            package_repository_id: repository_id,
            name: name.clone(),
            created_at: Utc::now(),
        ...
        packages
            .insert_version(&bunker_domain::npm_package::NpmPackageVersion {
                id: Uuid::new_v4(),
                npm_package_id: package.id,
                version: bunker_domain::npm_package::NpmVersion::parse("1.0.0").unwrap(),
                ...
                origin: bunker_domain::npm_package::NpmPackageOrigin::Local,
            })
            .await
            .unwrap();
