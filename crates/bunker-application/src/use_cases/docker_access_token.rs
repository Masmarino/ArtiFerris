...
use bunker_domain::docker_registry::{DockerGrantedScope, DockerScopeRequest, DockerTokenIssuerPort};
use bunker_domain::package_repository::PackageRepositoryQueryPort;
use bunker_domain::permission::{PermissionQueryPort, Role, organization_admin_bypass_role};
use bunker_domain::user::{User, UserRepositoryPort};
use uuid::Uuid;
...
mod tests {
    ...
    use bunker_domain::api_token::ApiToken;
    use bunker_domain::docker_registry::DockerAccessClaims;
    ...
    /// Mirrors bunker-api's org-admin bypass (`bunker_api::authz::effective_repository_role`) — implicit Admin on any repository in their own org, no explicit grant needed.
    #[tokio::test]
