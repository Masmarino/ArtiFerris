...
use bunker_domain::audit::{AuditEntry, AuditQueryFilter, EventPublisherPort, NpmPackageEvent, SecurityEvent};
use bunker_domain::error::{DomainError, EventStoreError};
use bunker_domain::npm_audit::{NpmAdvisory, NpmAuditPort};
use bunker_domain::npm_package::{NpmDistTag, NpmPackage, NpmPackageName, NpmPackageRepositoryPort, NpmPackageVersion, NpmVersion};
use bunker_domain::npm_remote::RemoteNpmRegistryPort;
use bunker_domain::package_repository::{PackageRepositoryQueryPort, PackageRepositorySummary};
use bunker_domain::storage::{StorageBackendPort, StorageError};
use uuid::Uuid;
...
impl NpmPackageRepositoryPort for FakePackages {
    async fn find_package(&self, repository_id: Uuid, name: &bunker_domain::npm_package::NpmPackageName) -> Result<Option<NpmPackage>, DomainError> {
    ...
    async fn list_versions_for_packages(&self, npm_package_ids: &[Uuid]) -> Result<Vec<bunker_domain::npm_package::NpmPackageVersionSummary>, DomainError> {
        Ok(self
            .versions
            .lock()
            .unwrap()
            ...
            .map(|v| bunker_domain::npm_package::NpmPackageVersionSummary {
                id: v.id,
...
impl RemoteNpmRegistryPort for FakeRemoteRegistry {
    async fn fetch_metadata(
        &self,
        _base_url: &str,
        package_name: &bunker_domain::npm_package::NpmPackageName,
...
pub struct FakeDependencyAuditResults {
    saved: Mutex<Vec<bunker_domain::npm_audit::DependencyAuditResult>>,
}
...
impl FakeDependencyAuditResults {
    ...
    pub fn saved(&self) -> Vec<bunker_domain::npm_audit::DependencyAuditResult> {
        self.saved.lock().unwrap().clone()
    }
}

#[async_trait]
impl bunker_domain::npm_audit::DependencyAuditRepositoryPort for FakeDependencyAuditResults {
    async fn save(&self, result: &bunker_domain::npm_audit::DependencyAuditResult) -> Result<(), DomainError> {
        self.saved.lock().unwrap().push(result.clone());
        Ok(())
    }

    async fn find_latest_for_version(&self, npm_package_version_id: Uuid) -> Result<Option<bunker_domain::npm_audit::DependencyAuditResult>, DomainError> {
        Ok(self
            .saved
            .lock()
            .unwrap()
    ...
    ) -> Result<Vec<bunker_domain::npm_audit::DependencyAuditResult>, DomainError> {
...
impl StorageBackendPort for FakeStorage {
    ...
    async fn read_stream(&self, repository_id: Uuid, path: &str) -> Result<bunker_domain::storage::ByteStream, StorageError> {
    ...
    async fn volume_space(&self) -> Result<bunker_domain::storage::VolumeSpace, StorageError> {
        Ok(bunker_domain::storage::VolumeSpace { total_bytes: 0, free_bytes: 0 })
    }
}
...
impl EventPublisherPort for FakeEvents {
    ...
    async fn publish_docker_event(&self, _event: bunker_domain::audit::DockerRegistryEvent, _package_repository_id: Uuid, _actor_id: Option<Uuid>) -> Result<(), EventStoreError> {
        Ok(())
    }
}
