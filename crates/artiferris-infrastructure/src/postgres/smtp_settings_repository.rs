...
mod tests {
    ...
    async fn get_returns_none_when_never_configured(pool: sqlx::PgPool) {
        ...
        assert_eq!(repo.get(artiferris_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap(), None);
    }
    ...
    async fn a_second_update_overwrites_the_first_rather_than_inserting_a_row(pool: sqlx::PgPool) {
        ...
        assert_eq!(count, 1);
        assert_eq!(repo.get(artiferris_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap().unwrap().host, "smtp2.example.com");
    }
