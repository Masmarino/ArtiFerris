...
mod tests {
    ...
    async fn get_returns_the_seeded_defaults_on_a_fresh_instance(pool: sqlx::PgPool) {
        ...
        let settings = repo.get(artiferris_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap();
    ...
    async fn update_then_get_round_trips_the_new_values(pool: sqlx::PgPool) {
        ...
        assert_eq!(repo.get(artiferris_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap(), updated);
    }

    #[sqlx::test(migrations = "../artiferris-infrastructure/migrations")]
    async fn a_second_update_overwrites_the_first_rather_than_inserting_a_row(pool: sqlx::PgPool) {
        let repo = PostgresSystemSettingsRepository::new(pool);
        repo.update(artiferris_domain::organization::PUBLIC_ORGANIZATION_ID, &SystemSettings { max_login_attempts: 5, login_attempt_window_seconds: 60, session_ttl_hours: 24, registration_enabled: true }).await.unwrap();
        repo.update(artiferris_domain::organization::PUBLIC_ORGANIZATION_ID, &SystemSettings { max_login_attempts: 20, login_attempt_window_seconds: 120, session_ttl_hours: 48, registration_enabled: false }).await.unwrap();

        let count: i64 = sqlx::query_scalar!("SELECT COUNT(*) FROM system_settings").fetch_one(&repo.pool).await.unwrap().unwrap();
        assert_eq!(count, 1);
        assert_eq!(repo.get(artiferris_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap().max_login_attempts, 20);
    }

    #[sqlx::test(migrations = "../artiferris-infrastructure/migrations")]
    async fn registration_enabled_round_trips_through_an_update(pool: sqlx::PgPool) {
        let repo = PostgresSystemSettingsRepository::new(pool);
        assert!(repo.get(artiferris_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap().registration_enabled, "must default to enabled");

        let mut settings = repo.get(artiferris_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap();
        settings.registration_enabled = false;
        repo.update(artiferris_domain::organization::PUBLIC_ORGANIZATION_ID, &settings).await.unwrap();

        assert!(!repo.get(artiferris_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap().registration_enabled);
    }
    ...
    async fn a_pre_migration_singleton_row_is_reattached_to_the_public_organization(pool: sqlx::PgPool) {
        ...
        repo.update(artiferris_domain::organization::PUBLIC_ORGANIZATION_ID, &SystemSettings { max_login_attempts: 7, login_attempt_window_seconds: 90, session_ttl_hours: 6, registration_enabled: false }).await.unwrap();
