...
use base64::Engine;
use bunker_domain::npm_package::{NpmPackageName, NpmVersion};
...
mod tests {
    ...
    use bytes::Bytes;
    use bunker_application::use_cases::api_token::{CreateApiTokenUseCase, ListApiTokensUseCase, RevokeApiTokenUseCase, hash_api_token};
    ...
    use bunker_infrastructure::http_npm_audit_client::HttpNpmAuditClient;
    use bunker_infrastructure::http_remote_npm_registry::HttpRemoteNpmRegistry;
    ...
    async fn test_state(pool: PgPool, root: &std::path::Path) -> NpmState {
        ...
        let api_tokens: Arc<dyn bunker_domain::api_token::ApiTokenRepositoryPort> = Arc::new(PostgresApiTokenRepository::new(pool.clone()));
        let organizations: Arc<dyn bunker_domain::organization::OrganizationRepositoryPort> = Arc::new(PostgresOrganizationRepository::new(pool.clone()));
        let npm_packages: Arc<dyn bunker_domain::npm_package::NpmPackageRepositoryPort> = Arc::new(PostgresNpmPackageRepository::new(pool.clone()));
        let npm_audit: Arc<dyn bunker_domain::npm_audit::NpmAuditPort> = Arc::new(HttpNpmAuditClient::new());
        let dependency_audits: Arc<dyn bunker_domain::npm_audit::DependencyAuditRepositoryPort> = Arc::new(PostgresDependencyAuditRepository::new(pool.clone()));
        let storage: Arc<dyn bunker_domain::storage::StorageBackendPort> = Arc::new(FilesystemStorageBackend::new(root.to_path_buf()));
        let events: Arc<dyn bunker_domain::audit::EventPublisherPort> = Arc::new(PostgresEventPublisher::new(pool.clone()));
        let remote_registry: Arc<dyn bunker_domain::npm_remote::RemoteNpmRegistryPort> = Arc::new(HttpRemoteNpmRegistry::new());
    ...
    /// Mirrors bunker-api's org-admin bypass (`bunker_api::authz::effective_repository_role`) — implicit Write on any repository in their own org, no explicit grant needed.
    #[sqlx::test(migrations = "../bunker-infrastructure/migrations")]
