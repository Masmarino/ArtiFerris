...
async fn lookup_user(
    ...
    let username = bunker_domain::user::Username::parse(&params.username).map_err(|_| not_found())?;
    let target = state
        .users
...
struct UserPermissionEntryResponse {
    ...
    format: bunker_domain::package_repository::RepositoryFormat,
    role: bunker_domain::permission::Role,
}
