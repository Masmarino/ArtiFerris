...
use bunker_application::error::ApplicationError;
use bunker_domain::audit::SecurityEvent;
use serde::{Deserialize, Serialize};

use bunker_domain::user::TokenIssuerPort;
...
async fn throttle_limits_for_username(state: &AppState, username: &str) -> (usize, std::time::Duration) {
    let Ok(username) = bunker_domain::user::Username::parse(username) else {
...
async fn sso_config(State(state): State<AppState>, resolved_org: ResolvedOrganization) -> Result<Json<SsoConfigResponse>, (StatusCode, Json<ErrorResponse>)> {
    ...
    let provider_type = config.map(|c| match c {
        bunker_domain::sso::IdentityProviderConfig::Ldap(_) => SsoProviderType::Ldap,
        bunker_domain::sso::IdentityProviderConfig::Oidc(_) => SsoProviderType::Oidc,
    });
    let settings = state
        .get_system_settings
        .execute(resolved_org.0.id)
...
async fn sso_oidc_login(
    ...
    let config = state
        ...
        .await
        .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Json(ErrorResponse { error: "internal error".to_string() })))?;
    let Some(bunker_domain::sso::IdentityProviderConfig::Oidc(oidc_config)) = config else {
...
async fn sso_ldap_login(
    ...
    let config = state
        ...
        .await
        .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Json(ErrorResponse { error: "internal error".to_string() })))?;
    let Some(bunker_domain::sso::IdentityProviderConfig::Ldap(ldap_config)) = config else {
...
mod tests {
    ...
    async fn enroll_and_confirm_totp(state: &AppState, user_id: uuid::Uuid) {
        ...
        let code = bunker_application::use_cases::mfa::generate_current_totp_code(&enrollment.secret_base32);
    ...
    async fn enroll_a_passkey(state: &AppState, user_id: uuid::Uuid) {
        state
            ...
            .insert(&bunker_domain::webauthn::WebauthnCredential {
                id: Uuid::new_v4(),
                user_id,
    ...
    async fn completing_mfa_verify_with_a_correct_totp_code_issues_a_working_session_token(pool: sqlx::PgPool) {
        ...
        let code = bunker_application::use_cases::mfa::generate_totp_code_after_step(&credential.secret, credential.last_used_step.unwrap());
        let response = app
            .clone()
            .oneshot(
                Request::builder()
    ...
    async fn enrolling_totp_during_mandatory_setup_issues_a_working_session_token(pool: sqlx::PgPool) {
        ...
        let code = bunker_application::use_cases::mfa::generate_current_totp_code(secret);
    ...
    async fn mandatory_totp_setup_is_rejected_once_the_account_already_has_a_passkey(pool: sqlx::PgPool) {
        ...
        state
            .webauthn_credentials
            .insert(&bunker_domain::webauthn::WebauthnCredential { id: uuid::Uuid::new_v4(), user_id, name: "YubiKey".to_string(), passkey_data: b"opaque".to_vec(), created_at: chrono::Utc::now() })
            .await
            .unwrap();
        let mfa_token = state.mfa_pending_token_issuer.issue(user_id, chrono::Duration::minutes(5)).unwrap();
        let app = build_router(state);
    ...
    async fn activating_with_a_valid_token_sets_a_working_password(pool: sqlx::PgPool) {
        ...
        state
            .user_invitations
            .upsert(&bunker_domain::invitation::UserInvitation {
                user_id,
                token_hash: bunker_application::use_cases::invitation::hash_invitation_token("raw-test-token"),
    ...
    async fn registering_while_registration_is_disabled_is_rejected(pool: sqlx::PgPool) {
        let state = AppState::build(pool, &test_config());
        let mut settings = state.get_system_settings.execute(bunker_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap();
        settings.registration_enabled = false;
        state.update_system_settings.execute(bunker_domain::organization::PUBLIC_ORGANIZATION_ID, settings).await.unwrap();
        let app = build_router(state);
    ...
    async fn sso_config_reports_registration_disabled_on_a_non_public_organization_even_when_the_setting_is_on(pool: sqlx::PgPool) {
        ...
        state
            .organizations
            .create(&bunker_domain::organization::Organization {
                id: Uuid::new_v4(),
                slug: bunker_domain::organization::OrganizationSlug::parse("acme").unwrap(),
    ...
    async fn login_failure_ips(state: &AppState) -> Vec<String> {
        let entries = state
            .query_audit_log
            .execute(bunker_domain::audit::AuditQueryFilter {
    ...
    async fn seed_ldap_config(state: &AppState, organization_id: Uuid) {
        state
            ...
            .set(
                organization_id,
                &bunker_domain::sso::IdentityProviderConfig::Ldap(bunker_domain::sso::LdapConfig {
    ...
    impl bunker_domain::sso::OidcAuthPort for FakeOidcAuth {
        async fn build_redirect(
            &self,
            _config: &bunker_domain::sso::OidcConfig,
            organization_id: Uuid,
            callback_url: &str,
            binding_secret: &str,
        ) -> Result<String, bunker_domain::error::DomainError> {
        ...
        async fn handle_callback(
            ...
            binding_secret: &str,
        ) -> Result<bunker_domain::sso::ExternalIdentity, bunker_domain::error::DomainError> {
            if raw_state != fake_state_token(expected_organization_id, binding_secret) {
                return Err(bunker_domain::error::DomainError::Infrastructure("oidc state token mismatch".to_string()));
            }
            Ok(bunker_domain::sso::ExternalIdentity { email: "florian@corp.example".to_string(), display_name: None })
        }
    }
    ...
    async fn create_non_public_organization(state: &AppState, slug: &str) -> Uuid {
        ...
        state
            ...
            .create(&bunker_domain::organization::Organization {
                id,
                slug: bunker_domain::organization::OrganizationSlug::parse(slug).unwrap(),
    ...
    async fn a_lower_throttle_limit_in_one_organization_does_not_affect_another(pool: sqlx::PgPool) {
        ...
        state
            ...
            .execute(
                acme_id,
                bunker_domain::system_settings::SystemSettings { max_login_attempts: 1, login_attempt_window_seconds: 300, session_ttl_hours: 12, registration_enabled: true },
            )
            .await
            .unwrap();
        let app = build_router(state);
    ...
    async fn a_login_token_carries_the_issuing_users_organizations_session_ttl(pool: sqlx::PgPool) {
        ...
        let acme_id = state.create_organization.execute("acme", "Acme Corp").await.unwrap();
        state.create_user.execute(acme_id, "acme-user", "sup3r-s3cret!", false).await.unwrap();
        state.update_system_settings.execute(acme_id, bunker_domain::system_settings::SystemSettings { max_login_attempts: 10, login_attempt_window_seconds: 300, session_ttl_hours: 1, registration_enabled: true }).await.unwrap();
        let app = build_router(state.clone());
