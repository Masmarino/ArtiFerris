...
async fn get_identity_provider(State(state): State<AppState>, user: AuthUser, Path(id): Path<Uuid>) -> Result<Json<serde_json::Value>, (StatusCode, Json<ErrorResponse>)> {
    ...
    let body = match config {
        ...
        Some(bunker_domain::sso::IdentityProviderConfig::Ldap(ldap)) => serde_json::json!({
            "type": "ldap",
        ...
        Some(bunker_domain::sso::IdentityProviderConfig::Oidc(oidc)) => serde_json::json!({
            "type": "oidc",
        ...
        SetIdentityProviderRequest::Ldap { server_url, bind_dn, bind_password, user_search_base, user_search_filter, email_attribute } => {
            let bind_password = match bind_password {
                ...
                _ => {
                    ...
                    match existing {
                        Some(bunker_domain::sso::IdentityProviderConfig::Ldap(ldap)) => ldap.bind_password,
            ...
            bunker_domain::sso::IdentityProviderConfig::Ldap(bunker_domain::sso::LdapConfig { server_url, bind_dn, bind_password, user_search_base, user_search_filter, email_attribute })
        }
        SetIdentityProviderRequest::Oidc { issuer_url, client_id, client_secret } => {
            let client_secret = match client_secret {
                ...
                _ => {
                    let existing = state.identity_providers.get(id).await.map_err(|e| application_error_response("failed to load existing identity provider", e.into()))?;
                    match existing {
                        Some(bunker_domain::sso::IdentityProviderConfig::Oidc(oidc)) => oidc.client_secret,
            ...
            bunker_domain::sso::IdentityProviderConfig::Oidc(bunker_domain::sso::OidcConfig { issuer_url, client_id, client_secret })
        }
    };
...
mod tests {
    ...
    async fn configuring_oidc_replaces_an_existing_ldap_configuration(pool: sqlx::PgPool) {
        ...
        assert!(matches!(stored, Some(bunker_domain::sso::IdentityProviderConfig::Oidc(_))), "the old LDAP config must be fully replaced, not merged");
    }
    ...
    async fn an_organization_admin_can_invite_a_member_into_their_own_organization(pool: sqlx::PgPool) {
        ...
        assert_eq!(response.status(), StatusCode::CREATED);
        let created = state.users.find_by_username(&bunker_domain::user::Username::parse("newmember").unwrap()).await.unwrap().unwrap();
    ...
    async fn inviting_a_member_ignores_a_forged_is_super_admin_field(pool: sqlx::PgPool) {
        ...
        assert_eq!(response.status(), StatusCode::CREATED);
        let created = state.users.find_by_username(&bunker_domain::user::Username::parse("sneaky").unwrap()).await.unwrap().unwrap();
