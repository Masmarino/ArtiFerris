...
use bunker_application::use_cases::smtp::UpdateSmtpSettingsInput;
use bunker_domain::audit::{AuditEntry, AuditQueryFilter};
use bunker_domain::email::SmtpSecurity;
use bunker_domain::health::ComponentHealth;
use bunker_domain::package_repository::{RepositoryFormat, RepositoryType};
use bunker_domain::permission::Role;
use bunker_domain::system_settings::SystemSettings;
...
async fn organization_repository_ids(state: &AppState, organization_id: Uuid) -> Result<std::collections::HashSet<Uuid>, bunker_domain::error::EventStoreError> {
...
async fn get_stats(State(state): State<AppState>, user: AuthUser, Query(scope): Query<OrgScopeParams>) -> Result<Json<AdminStatsResponse>, (StatusCode, Json<ErrorResponse>)> {
    ...
    let stats = if let Some(org) = target_org.or_else(|| (!user.is_super_admin).then_some(user.organization_id)) {
        ...
        bunker_application::use_cases::admin::AdminStats { total_users, total_repositories: org_repository_ids.len(), total_active_permissions }
    } else {
...
async fn import_configuration(State(state): State<AppState>, user: AuthUser, Json(body): Json<ConfigurationImportRequest>) -> Result<Json<ImportReportResponse>, (StatusCode, Json<ErrorResponse>)> {
    require_super_admin(&user).map_err(|status| (status, Json(ErrorResponse { error: "forbidden".to_string() })))?;
    let import = bunker_application::use_cases::admin::ConfigurationImport {
        users: body
            .users
            .into_iter()
            .map(|u| bunker_application::use_cases::admin::ExportedUser { id: u.id, username: u.username, is_super_admin: u.is_super_admin, created_at: u.created_at, email: u.email })
            .collect(),
        repositories: body
            .repositories
            .into_iter()
            .map(|r| bunker_application::use_cases::admin::ExportedRepository {
                id: r.id,
                name: r.name,
                format: r.format,
                repo_type: r.repo_type,
        ...
        permissions: body
            .permissions
            .into_iter()
            .map(|p| bunker_application::use_cases::admin::ExportedPermission { user_id: p.user_id, repository_id: p.repository_id, role: p.role })
            .collect(),
...
mod tests {
    ...
    async fn the_audit_log_can_exclude_security_events(pool: sqlx::PgPool) {
        ...
        for _ in 0..5 {
            state
                ...
                .execute(
                    bunker_domain::audit::SecurityEvent::LoginFailed { username: "flooder".to_string(), ip: "127.0.0.1".to_string() },
                    None,
                )
                .await
                .unwrap();
    ...
    async fn an_organization_admin_sees_a_package_event_for_their_own_repository(pool: sqlx::PgPool) {
        ...
        state.users.set_organization_admin(org_admin_id, true).await.unwrap();
        state.create_repository.execute(acme_id, "acme-repo", RepositoryFormat::Npm, RepositoryType::Hosted, None, None, None, org_admin_id).await.unwrap();
        // No explicit permission grant — relies on bunker-npm's own org-admin authz bypass.
    ...
    async fn an_organization_admin_does_not_see_unattributable_login_failures(pool: sqlx::PgPool) {
        ...
        state
            .record_security_event
            .execute(bunker_domain::audit::SecurityEvent::LoginFailed { username: "flooder".to_string(), ip: "127.0.0.1".to_string() }, None)
            .await
            .unwrap();
        let org_admin_token = state.authenticate_user.execute("org-admin", "sup3r-s3cret!").await.unwrap();
        let app = build_router(state);
    ...
    async fn admin_updates_system_settings_and_it_takes_effect_immediately(pool: sqlx::PgPool) {
        ...
        assert_eq!(response.status(), axum::http::StatusCode::NO_CONTENT);
        let persisted = state.get_system_settings.execute(bunker_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap();
    ...
    async fn an_organization_admins_system_settings_update_does_not_affect_another_organization(pool: sqlx::PgPool) {
        ...
        let public_settings = state.get_system_settings.execute(bunker_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap();
        assert_eq!(public_settings, bunker_domain::system_settings::SystemSettings::defaults(), "the public organization's settings must be untouched by another organization's update");
    }
    ...
    async fn a_super_admin_can_target_a_specific_organizations_system_settings_via_the_query_param(pool: sqlx::PgPool) {
        ...
        let public_settings = state.get_system_settings.execute(bunker_domain::organization::PUBLIC_ORGANIZATION_ID).await.unwrap();
        assert_eq!(public_settings, bunker_domain::system_settings::SystemSettings::defaults(), "the public organization must be untouched");
    }
