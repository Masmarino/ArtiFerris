...
mod tests {
    ...
    use bunker_application::use_cases::mfa::generate_current_totp_code;
    ...
    async fn disabling_totp_requires_the_current_password(pool: sqlx::PgPool) {
        ...
        assert_eq!(state.get_mfa_status.execute(state.users.find_by_username(&bunker_domain::user::Username::parse("florian").unwrap()).await.unwrap().unwrap().id).await.unwrap().totp_enabled, false);
    }
