...
use bunker_application::use_cases::list_repository_packages::{RepositoryPackageTree, VulnerabilitySummary};
use bunker_domain::docker_registry::DockerImageName;
        ...
        for member_id in &group_members {
            let member = state
                ...
                .ok_or_else(|| {
                    application_error_response(
                        ...
                        bunker_domain::error::DomainError::UnknownGroupMember(*member_id).into(),
                    )
                })?;
            ...
            if member.organization_id != organization_id {
                return Err(application_error_response(
                    "failed to create repository",
                    bunker_domain::error::DomainError::GroupMemberOrganizationMismatch(*member_id).into(),
                ));
            }
            if member.format != body.format {
                return Err(application_error_response(
                    "failed to create repository",
                    bunker_domain::error::DomainError::GroupMemberFormatMismatch(*member_id).into(),
                ));
            }
        }
    }
...
impl From<bunker_domain::npm_audit::DependencyAuditResult> for DependencyAuditResultResponse {
    fn from(result: bunker_domain::npm_audit::DependencyAuditResult) -> Self {
        Self {
...
impl From<bunker_domain::docker_scan::DockerImageScanResult> for DockerImageScanResultResponse {
    fn from(result: bunker_domain::docker_scan::DockerImageScanResult) -> Self {
        Self {
            scanned_at: result.scanned_at,
...
mod tests {
    ...
    async fn a_repository_name_is_reusable_after_deletion(pool: sqlx::PgPool) {
        let state = AppState::build(pool, &test_config());
        let token = bearer(&state, Uuid::parse_str("00000000-0000-0000-0000-000000000001").unwrap(), "admin", "sup3r-s3cret!", true).await;
        let admin_id = state.users.find_by_username(&bunker_domain::user::Username::parse("admin").unwrap()).await.unwrap().unwrap().id;
        let first_id = state
            .create_repository
    ...
    async fn browsing_an_npm_repository_lists_its_packages_and_versions(pool: sqlx::PgPool) {
        use bunker_domain::npm_package::{NpmPackage, NpmPackageName, NpmPackageOrigin, NpmPackageVersion, NpmVersion};
    ...
    async fn browsing_a_docker_repository_lists_its_images_and_tags(pool: sqlx::PgPool) {
        use bunker_domain::docker_registry::{Digest, DockerImageName, DockerManifest, DockerMediaType};
    ...
    async fn a_read_only_user_cannot_delete_an_npm_package(pool: sqlx::PgPool) {
        use bunker_domain::npm_package::{NpmPackage, NpmPackageName};
