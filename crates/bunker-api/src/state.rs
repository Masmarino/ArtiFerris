...
use bunker_application::use_cases::admin::{
...
use bunker_application::use_cases::branding::{BrandingDefaults, ClearBrandingFaviconUseCase, ClearBrandingLogoUseCase, GetBrandingUseCase, SetBrandingFaviconUseCase, SetBrandingLogoUseCase};
use bunker_application::use_cases::docker_manifest_delete::{DeleteDockerImageUseCase, DeleteManifestUseCase};
use bunker_application::use_cases::docker_scan::{GetDockerImageScanResultUseCase, ScanDockerImageUseCase};
use bunker_application::use_cases::list_repository_packages::ListRepositoryPackagesUseCase;
use bunker_application::use_cases::npm_audit::AuditNpmPackageUseCase;
use bunker_application::use_cases::npm_dependency_scan::{GetDependencyAuditResultUseCase, ScanDependencyTreeUseCase};
use bunker_application::use_cases::npm_unpublish::UnpublishNpmPackageUseCase;
use bunker_application::use_cases::organization::CreateOrganizationUseCase;
use bunker_application::use_cases::package_details::{GetDockerImageDetailsUseCase, GetNpmPackageDetailsUseCase};
use bunker_application::use_cases::package_repository::{
...
use bunker_application::use_cases::permission::{GrantPermissionUseCase, RevokePermissionUseCase};
use bunker_application::use_cases::invitation::{ActivateAccountUseCase, InviteUserUseCase, ResendInvitationUseCase};
use bunker_application::use_cases::mfa::{
...
use bunker_application::use_cases::smtp::{GetSmtpSettingsUseCase, SendTestEmailUseCase, UpdateSmtpSettingsUseCase};
use bunker_application::use_cases::webauthn::{
...
use bunker_application::use_cases::user::{AuthenticateUserUseCase, ChangePasswordUseCase, CreateUserUseCase, DeleteUserUseCase, SetOrganizationAdminUseCase, SetSuperAdminUseCase};
use bunker_application::use_cases::registration::RegisterPublicUserUseCase;
use bunker_application::use_cases::sso::ProvisionSsoUserUseCase;
use bunker_domain::api_token::ApiTokenRepositoryPort;
use bunker_domain::audit::EventPublisherPort;
use bunker_domain::npm_audit::NpmAuditPort;
use bunker_domain::npm_package::NpmPackageRepositoryPort;
...
use bunker_domain::storage::StorageBackendPort;
use bunker_domain::user::{TokenIssuerPort, UserRepositoryPort};
use bunker_domain::sso::{IdentityProviderRepositoryPort, LdapAuthPort, OidcAuthPort};
use bunker_domain::branding::BrandingAsset;
use bunker_infrastructure::argon2_hasher::Argon2PasswordHasher;
use bunker_infrastructure::branding_defaults;
use bunker_infrastructure::filesystem_storage::FilesystemStorageBackend;
use bunker_infrastructure::jwt_mfa_pending_token_issuer::JwtMfaPendingTokenIssuer;
use bunker_infrastructure::jwt_token_issuer::JwtTokenIssuer;
use bunker_infrastructure::postgres::api_token_repository::PostgresApiTokenRepository;
use bunker_infrastructure::postgres::backup_code_repository::PostgresBackupCodeRepository;
use bunker_infrastructure::postgres::event_publisher::PostgresEventPublisher;
use bunker_infrastructure::postgres::health::PostgresHealthCheck;
use bunker_infrastructure::postgres::docker_image_scan_repository::PostgresDockerImageScanRepository;
use bunker_infrastructure::postgres::metrics_snapshot_repository::PostgresMetricsSnapshotRepository;
use bunker_infrastructure::postgres::npm_dependency_audit_repository::PostgresDependencyAuditRepository;
use bunker_infrastructure::postgres::npm_package_repository::PostgresNpmPackageRepository;
use bunker_infrastructure::postgres::organization_repository::PostgresOrganizationRepository;
use bunker_infrastructure::postgres::package_repository_store::PostgresPackageRepositoryStore;
use bunker_infrastructure::postgres::permission_store::PostgresPermissionStore;
use bunker_infrastructure::postgres::smtp_settings_repository::PostgresSmtpSettingsRepository;
use bunker_infrastructure::postgres::system_settings_repository::PostgresSystemSettingsRepository;
use bunker_infrastructure::postgres::totp_credential_repository::PostgresTotpCredentialRepository;
use bunker_infrastructure::postgres::user_invitation_repository::PostgresUserInvitationRepository;
use bunker_infrastructure::postgres::user_repository::PostgresUserRepository;
use bunker_infrastructure::postgres::webauthn_credential_repository::PostgresWebauthnCredentialRepository;
use bunker_infrastructure::ldap3_auth_adapter::Ldap3AuthAdapter;
use bunker_infrastructure::openidconnect_auth_adapter::OpenidConnectAuthAdapter;
use bunker_infrastructure::postgres::identity_provider_repository::PostgresIdentityProviderRepository;
use bunker_infrastructure::smtp_email_sender::SmtpEmailSender;
use sqlx::PgPool;
...
pub struct AppState {
    ...
    /// Exposed so `main.rs` can reuse the same adapters for `bunker_npm`/`bunker_docker` state.
    ...
    pub docker_blobs: Arc<dyn bunker_domain::docker_registry::DockerBlobStorePort>,
    pub docker_manifests: Arc<dyn bunker_domain::docker_registry::DockerManifestRepositoryPort>,
    pub docker_uploads: Arc<dyn bunker_application::use_cases::docker_upload::DockerUploadSessionPort>,
    ...
    pub sweep_retention: Arc<bunker_application::use_cases::retention::SweepRetentionUseCase>,
    ...
    pub user_invitations: Arc<dyn bunker_domain::invitation::UserInvitationPort>,
    pub totp_credentials: Arc<dyn bunker_domain::mfa::TotpCredentialPort>,
    ...
    pub webauthn_credentials: Arc<dyn bunker_domain::webauthn::WebauthnCredentialPort>,
...
impl AppState {
    pub fn build(pool: PgPool, config: &Config) -> Self {
        ...
        let npm_audit: Arc<dyn NpmAuditPort> = Arc::new(bunker_infrastructure::http_npm_audit_client::HttpNpmAuditClient::new());
        let dependency_audits: Arc<dyn bunker_domain::npm_audit::DependencyAuditRepositoryPort> =
            Arc::new(PostgresDependencyAuditRepository::new(pool.clone()));
        let public_registry: Arc<dyn bunker_domain::npm_remote::RemoteNpmRegistryPort> =
            Arc::new(bunker_infrastructure::http_remote_npm_registry::HttpRemoteNpmRegistry::new());
        let docker_blobs: Arc<dyn bunker_domain::docker_registry::DockerBlobStorePort> = Arc::new(
            bunker_infrastructure::filesystem_docker_blob_store::FilesystemDockerBlobStore::new(
                pool.clone(),
                std::path::Path::new(&config.storage_root).join("docker-blobs"),
            ),
        );
        let docker_manifests: Arc<dyn bunker_domain::docker_registry::DockerManifestRepositoryPort> =
            Arc::new(bunker_infrastructure::postgres::docker_manifest_repository::PostgresDockerManifestRepository::new(pool.clone()));
        let docker_uploads: Arc<dyn bunker_application::use_cases::docker_upload::DockerUploadSessionPort> = Arc::new(
            bunker_infrastructure::postgres::docker_upload_session_repository::PostgresDockerUploadSessionRepository::new(
                pool.clone(),
                std::path::Path::new(&config.storage_root).join("docker-uploads"),
            ),
        );
        let docker_image_scans: Arc<dyn bunker_domain::docker_scan::DockerImageScanRepositoryPort> =
        ...
        let docker_scanner: Arc<dyn bunker_domain::docker_scan::DockerImageScannerPort> =
            Arc::new(bunker_infrastructure::trivy_docker_image_scanner::TrivyDockerImageScanner::new(format!("127.0.0.1:{registry_port}")));
        let docker_token_issuer: Arc<dyn bunker_domain::docker_registry::DockerTokenIssuerPort> =
            Arc::new(bunker_infrastructure::jwt_docker_token_issuer::JwtDockerTokenIssuer::new(config.jwt_secret.clone()));
        let metrics_snapshots: Arc<dyn bunker_domain::metrics_snapshot::MetricsSnapshotRepositoryPort> =
        ...
        let system_settings: Arc<dyn bunker_domain::system_settings::SystemSettingsPort> = Arc::new(PostgresSystemSettingsRepository::new(pool.clone()));
        let smtp_settings: Arc<dyn bunker_domain::email::SmtpSettingsPort> = Arc::new(PostgresSmtpSettingsRepository::new(pool.clone(), config.secrets_encryption_key.clone()));
        let branding: Arc<dyn bunker_domain::branding::BrandingPort> = Arc::new(bunker_infrastructure::postgres::branding_repository::PostgresBrandingRepository::new(pool.clone()));
        let email_sender: Arc<dyn bunker_domain::email::EmailPort> = Arc::new(SmtpEmailSender::new(smtp_settings.clone(), branding.clone()));
        let user_invitations: Arc<dyn bunker_domain::invitation::UserInvitationPort> = Arc::new(PostgresUserInvitationRepository::new(pool.clone()));
        let totp_credentials: Arc<dyn bunker_domain::mfa::TotpCredentialPort> = Arc::new(PostgresTotpCredentialRepository::new(pool.clone(), config.secrets_encryption_key.clone()));
        let backup_codes: Arc<dyn bunker_domain::mfa::BackupCodePort> = Arc::new(PostgresBackupCodeRepository::new(pool.clone()));
        ...
        let sweep_retention = Arc::new(bunker_application::use_cases::retention::SweepRetentionUseCase::new(
