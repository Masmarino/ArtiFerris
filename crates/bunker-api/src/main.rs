...
async fn main() {
    ...
    let pool = bunker_infrastructure::postgres::connect(&config.database_url, config.db_max_connections).await.expect("failed to connect to postgres");
    bunker_infrastructure::postgres::run_migrations(&pool).await.expect("failed to run migrations");
    ...
    tracing::info!("bunker-api listening on {}", config.bind_addr);
...
pub fn build_router_with_cors(state: AppState, cors_allowed_origin: Option<String>, jwt_secret: String, docker_token_realm: String, public_url: String) -> Router {
    ...
    let router = Router::new()
        ...
        .nest_service("/npm", bunker_npm::router(npm_state))
        .nest_service("/v2", bunker_docker::router(docker_state))
...
fn build_npm_state(state: &AppState) -> bunker_npm::NpmState {
    let remote_registry: Arc<dyn bunker_domain::npm_remote::RemoteNpmRegistryPort> =
    ...
    bunker_npm::NpmState {
        ...
        metadata: Arc::new(bunker_application::use_cases::npm_metadata::GetNpmPackageMetadataUseCase::new(
            state.npm_packages.clone(),
            state.repositories.clone(),
            remote_registry.clone(),
        )),
        download: Arc::new(bunker_application::use_cases::npm_download::DownloadNpmTarballUseCase::new(
            state.npm_packages.clone(),
            state.storage.clone(),
            remote_registry.clone(),
            state.repositories.clone(),
        )),
        unpublish: Arc::new(bunker_application::use_cases::npm_unpublish::UnpublishNpmPackageUseCase::new(
            state.npm_packages.clone(),
            state.storage.clone(),
            state.events.clone(),
        )),
        deprecate: Arc::new(bunker_application::use_cases::npm_deprecate::DeprecateNpmVersionUseCase::new(
            state.npm_packages.clone(),
            state.events.clone(),
        )),
        set_dist_tag: Arc::new(bunker_application::use_cases::npm_dist_tags::SetDistTagUseCase::new(
            state.npm_packages.clone(),
            state.events.clone(),
        )),
        delete_dist_tag: Arc::new(bunker_application::use_cases::npm_dist_tags::DeleteDistTagUseCase::new(state.npm_packages.clone())),
        list_dist_tags: Arc::new(bunker_application::use_cases::npm_dist_tags::ListDistTagsUseCase::new(state.npm_packages.clone())),
        search: Arc::new(bunker_application::use_cases::npm_search::SearchNpmPackagesUseCase::new(state.npm_packages.clone())),
        bulk_audit: Arc::new(bunker_application::use_cases::npm_audit::BulkAuditNpmPackagesUseCase::new(state.npm_audit.clone())),
...
fn build_docker_state(state: &AppState, jwt_secret: &str, token_realm: &str) -> bunker_docker::DockerState {
    let remote: Arc<dyn bunker_domain::docker_remote::RemoteDockerRegistryPort> =
        Arc::new(bunker_infrastructure::http_remote_docker_registry::HttpRemoteDockerRegistry::new());
    let token_issuer: Arc<dyn bunker_domain::docker_registry::DockerTokenIssuerPort> =
        Arc::new(bunker_infrastructure::jwt_docker_token_issuer::JwtDockerTokenIssuer::new(jwt_secret.to_string()));
    let list_catalog = Arc::new(bunker_application::use_cases::docker_list::ListCatalogUseCase::new(state.docker_manifests.clone()));

    bunker_docker::DockerState {
        repositories: state.repositories.clone(),
        permissions: state.permissions.clone(),
        organizations: state.organizations.clone(),
        hangar_base_domain: state.hangar_base_domain.clone(),
        ...
        start_upload: Arc::new(bunker_application::use_cases::docker_upload::StartBlobUploadUseCase::new(state.docker_uploads.clone())),
        patch_upload: Arc::new(bunker_application::use_cases::docker_upload::PatchBlobUploadUseCase::new(state.docker_uploads.clone())),
        complete_upload: Arc::new(bunker_application::use_cases::docker_upload::CompleteBlobUploadUseCase::new(
            state.docker_uploads.clone(),
            state.docker_blobs.clone(),
        )),
        monolithic_upload: Arc::new(bunker_application::use_cases::docker_upload::MonolithicBlobUploadUseCase::new(state.docker_blobs.clone())),
        put_manifest: Arc::new(bunker_application::use_cases::docker_manifest_put::PutManifestUseCase::new(
            state.docker_manifests.clone(),
            state.docker_blobs.clone(),
            state.repositories.clone(),
            state.events.clone(),
        )),
        get_manifest: Arc::new(bunker_application::use_cases::docker_manifest_get::GetManifestUseCase::new(
            state.docker_manifests.clone(),
            state.repositories.clone(),
            remote.clone(),
        )),
        cache_proxied_manifest: Arc::new(bunker_application::use_cases::docker_manifest_cache::CacheProxiedManifestUseCase::new(
            state.docker_manifests.clone(),
        )),
        get_blob: Arc::new(bunker_application::use_cases::docker_blob_get::GetBlobUseCase::new(
            state.docker_blobs.clone(),
            state.docker_manifests.clone(),
            state.repositories.clone(),
            remote,
        )),
        delete_manifest: Arc::new(bunker_application::use_cases::docker_manifest_delete::DeleteManifestUseCase::new(
            state.docker_manifests.clone(),
            state.docker_blobs.clone(),
            state.events.clone(),
        )),
        list_tags: Arc::new(bunker_application::use_cases::docker_list::ListTagsUseCase::new(state.docker_manifests.clone())),
        list_catalog,
        list_registry_catalog: Arc::new(bunker_application::use_cases::docker_list::ListDockerRegistryCatalogUseCase::new(
            state.repositories.clone(),
            state.permissions.clone(),
            state.docker_manifests.clone(),
        )),
...
mod tests {
    ...
    async fn a_docker_blob_uploads_location_header_resolves_back_into_the_nested_v2_router(pool: sqlx::PgPool) {
        use bunker_domain::package_repository::{RepositoryFormat, RepositoryType};
        ...
        let digest = bunker_domain::docker_registry::Digest::of(&blob_bytes);
